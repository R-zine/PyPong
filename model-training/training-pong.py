    ...
    json.dump(
        {
            ...
            "ballR": BALL_R / ASPECT,
            "paddleWidth": PADDLE_WIDTH / ASPECT,
            "paddleHeight": PADDLE_HEIGHT / ASPECT,
            "velocity": VEL / ASPECT,
            "maxYVelocity": MAX_Y_VEL / ASPECT,
            "yDiffFactor": Y_DIFF_FACTOR,
            "ballStartVelocity": BALL_START_VEL / ASPECT,
            "maxBalls": MAX_BALLS_NUMBER,
        },
        f,
    )
...
def handle_collision(ball: Ball, paddle: Paddle) -> bool:
    if (
        ball.x_vel < 0
        and not ball.dead
        and ball.x <= paddle.x + PADDLE_WIDTH
        and ball.y >= paddle.y
...
def gameLoop(genome, config):
    ...
    while running:
        ...
        nearest_ball_index = balls.index(
            min(balls, key=lambda x: x.x if x.x_vel < 0 else WIDTH)
        )

        output = net.activate(
            (
                paddle.y + PADDLE_HEIGHT // 2,
                balls[nearest_ball_index].y,
                balls[nearest_ball_index].x,
                balls[nearest_ball_index].y_vel,
            )
        )
        ...
        pygame.draw.rect(
            win, "white", (paddle.x, paddle.y, PADDLE_WIDTH, PADDLE_HEIGHT)
        )
